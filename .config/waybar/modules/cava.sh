...
HELP
    ...
    *)
    ...
    # Use the max_range set in cava config to determine how many numerical steps cava will output.
    # This corresponds to the ascii_max_range in cava's config.
    # If not explicitly set by the user, default to 8 (like cava's default)
    current_max_range=${cava_range:-8} # Use 8 as a good default for range if not specified

    # Create dictionary to replace char with bar
    dict="s/;//g"

    # Handle standby mode: stbAscii and asciiBar are already defined above this section.
    # Add the standby replacement rule first.
    stbAscii=$(printf '0%.0s' $(seq 1 "${bar_width}"))
    [ -n "${asciiBar}" ] || asciiBar="${stbAscii//0/${stbBar}}"
    dict="$dict;s/${stbAscii}/${asciiBar}/g" # This maps the standby pattern

    # Map each number (from 0 up to bar_length - 1) to its corresponding character
    for (( i=0; i < bar_length; i++ )); do
        dict="$dict;s/$i/${bar:$i:1}/g"
    done

    # Ensure any number higher than the character index gets mapped to the highest character.
    # The highest character index is bar_length - 1.
    # Any number from bar_length up to current_max_range should map to the last character.
    if [ "${bar_length}" -gt 0 ]; then
        last_char_index=$((bar_length - 1))
        last_char="${bar:$last_char_index:1}"

        # Loop from bar_length up to the maximum expected range from cava
        # This covers all numbers that cava might output that don't have a specific char mapping
        for (( i=bar_length; i <= current_max_range; i++ )); do
            dict="$dict;s/$i/${last_char}/g"
        done
    fi
    ...
    cat >"$config_file" <<EOF
[general]
bars = ${bar_width:-${bar_length}} # Default bar_width if not set by user
sleep_timer = 1
[input]
method = pulse
source = auto
...
data_format = ascii
ascii_max_range = ${current_max_range} # Use the current_max_range we determined
EOF
